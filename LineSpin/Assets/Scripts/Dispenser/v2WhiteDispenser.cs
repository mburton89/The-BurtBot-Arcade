...
public class v2WhiteDispenser : MonoBehaviour {
	...
	//new spawn points
	private Vector3 highLeftSpawnLocation = new Vector3 (-10.5f, 8f, 0); 
	private Vector3 highRightSpawnLocation = new Vector3 (10.5f, 8f, 0); 
	...
	public void fireFromHighRight(){
		if(canFire){
			//AudioSource.PlayClipAtPoint(F, transform.position);  
			var liveProjectile = (Projectile)Instantiate (projectile, highRightSpawnLocation, transform.rotation); 
			liveProjectile.Initialize (gameObject, downLeft, new Vector2(-1, -1));  
			activeProjectiles ++;
			liveProjectile.GetComponent<SpriteRenderer>().color = line.GetComponent<SpriteRenderer>().color;  
		}
	...
	public void initiateDoubleShot1Sequence(){
		StartCoroutine(DoubleShot1Co());                
	}
	
	private IEnumerator DoubleShot1Co(){
		fireFromLeft1();
		fireFromLeft2();
		yield return new WaitForSeconds (waitTime2); 
		fireFromRight1();
