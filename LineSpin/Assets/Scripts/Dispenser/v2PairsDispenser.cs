...
public class v2PairsDispenser : MonoBehaviour {
...
//	private Vector3 Right2ProjectileSpawnLocation = new Vector3 (9.522f, 5, 0);
	...
	//new spawn points
	private Vector3 highLeftSpawnLocation = new Vector3 (-10.5f, 10f, 0); 
	private Vector3 highRightSpawnLocation = new Vector3 (10.5f, 10f, 0); 
	...
	public void fireRandomSequence(){
		float sequenceNumber = random.Next (3, 4);
		if(sequenceNumber == 1){   
			initiateRandomPair2();
	...
	public void fireFromHighLeft(){
		if(canFire){
			//AudioSource.PlayClipAtPoint(F, transform.position);  
			var liveProjectile = (Projectile)Instantiate (projectile, highLeftSpawnLocation, transform.rotation); 
			liveProjectile.Initialize (gameObject, downRight, new Vector2(1, -1));  
			activeProjectiles ++;
			liveProjectile.GetComponent<SpriteRenderer>().color = line.GetComponent<SpriteRenderer>().color;  
		}
	...
	public void fireFromHighRight(){
		if(canFire){
			//AudioSource.PlayClipAtPoint(F, transform.position);  
			var liveProjectile = (Projectile)Instantiate (projectile2, highRightSpawnLocation, transform.rotation); 
			liveProjectile.Initialize (gameObject, downLeft, new Vector2(-1, -1));  
			activeProjectiles ++;
			liveProjectile.GetComponent<SpriteRenderer>().color = line.GetComponent<SpriteRenderer>().color;  
		}
	...
	public void fireLeftHorizontal(){
		fireFromLeft1();
		fireFromLeft2();
	}
	
	public void fireLeftVertical(){
		fireFromLeft1();
		fireFromHighLeft();
	}
	
	public void fireRightHorizontal(){
		fireFromRight1();
		fireFromRight2();
	}
	
	public void fireRightVertical(){
		fireFromRight2();
		fireFromHighRight();
	}

	public void initiateRandomPair(){
		StartCoroutine(RandomPairCo());                
	}
	
	private IEnumerator RandomPairCo(){
		float sequenceNumber = random.Next (3, 5);
		if(sequenceNumber == 1){   
			fireLeftHorizontal();
		}else if(sequenceNumber == 2){
			fireRightHorizontal();
	...
	public void initiateRandomPair2(){
		StartCoroutine(RandomPairCo2());                
	}
	
	private IEnumerator RandomPairCo2(){
